output "api_url" {
  value = element(compact(concat(["default"], aws_api_gateway_deployment.deployment.*.invoke_url)), 0)
}
...
output "api_gw_message_resource_id" {
  ...
  value = element(compact(concat(["default"], aws_api_gateway_resource.messages_resource.*.id)), 0)
